...
export function CursorGrid({
  ...
  // Hook into the theme context for reactive color updates
  const { isDark } = useTheme();
  const finalColor = color || (isDark ? '#38bdf8' : '#3b82f6');
  const finalMaxOpacity = maxOpacity !== undefined ? maxOpacity : (isDark ? 0.55 : 0.28);
  const finalFillOpacity = fillOpacity !== undefined ? fillOpacity : (isDark ? 0.06 : 0.02);
  ...
  useEffect(() => {
    wakeRef.current?.();
  }, [gridOpacity, finalColor, lineWidth, finalMaxOpacity, finalFillOpacity, cellRadius]);
